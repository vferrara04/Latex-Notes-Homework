...
\begin{enumerate}
    ...
    Total number of posibilities is $2^n$\\
    The number of ways to get at least tow heads is two first pick two heads from our $n$ coin flips which is $\binom{n}{2}$ then the remaing $n-2$ flips have 2 choices thus $2^{n-2}$ so the total number of ways is
    $\binom{n}{2}\times 2^{n-2}$ thus\\
    $\P(\t{at least two heads have appeared to date})=\dfrac{\binom{n}{2}\times 2^{n-2}}{2^n}$
\end{enumerate}
\end{document}