...
\begin{problemproof}
    ...
    Thus, let $\{b_1,\dots,b_n\}$ be a basis for $K$ has a vector space over $F$.

    Thus, $k=a_1b_1+\dots+a_nb_n$ for $a_i \in F$ for all $k \in k$.

    Thus, $kl=a_1lb_1+\dots+a_nlb_n$ for all $l \in L$.

    Thus, $kl=\sum_i (a_il)b_i$ and since $l \in L$ and $a_i \in F \subset L$ we get taht $a_i,l \in L$.

    Thus, $kl=\sum_i l_i b_i$ for $l_i \in L$.
