...
\begin{enumerate}[(a)]
   ...
   Thus every element in $S^{-1}R$ can written in the form of $r/s'$ for $s' \in S'$
   ...
   Now for $\Z \diagup 60\Z$ using the proof above all the units are things that are co prime with $60$. Thus, we only have to look at things that divide 60, which are in the form $2^a3^b5^c$ where $a,b,c \geq 0$.
    ...
    Assume every prime ideal of $A$ is maximal. Let $p,q \in Spec(A)$. Since $p,q$ are maximal you can find an element $f \in A$ s.t. $f \in p$ and $f \notin q$. Thus $q \in X_f$ but $p$ is not. You can do the same for $q$ and get $p \in X_g$ and $q \notin X_g$.

    Consider $X_g \cap X_f$ from (a) we get $X_g \cap X_f = X_{fg}$. Since $p,q$ are maximal this means that $fg$ is in every other maximal ideal thus $X_{fg}=\emptyset$. Thus $Spec(A)$ is Hausdorff.
\end{enumerate}
