...
\propp{
    ...
    \noindent So, equivalence classes are connected:

    \noindent Fix $x_0 \in C$ for equivalence class $C$. For each $y \in C$ there is a connected $A_y$ containing $x_0,y$. Know $A_y \subseteq C$, so $\displaystyle\bigcup_{y \in C}A_c=C$.

    \noindent $\displaystyle\bigcap_{y \in C}A_y$ contains $x_0$ so $C$ is connected.

    \noindent If $A$ connected, contains equivalence class $C$, by def every $z \in A$ is equivalent to every $y \in C$, so $A \subseteq C$ and $A=C$.

    \noindent $X$ is connected $\iff$ $X$ has exactly one or zero connected components.
}
